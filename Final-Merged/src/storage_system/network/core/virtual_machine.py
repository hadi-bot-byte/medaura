...
class VirtualMachine:
    ...
    def _heartbeat_monitor(self):
        ...
        while True:
            ...
            for node_id, status in list(self.known_nodes.items()):
                if current_time - status.last_heartbeat > 15:  # 15 second timeout
                    status.is_alive = False
                    print(f"[VM {self.config.node_id}] Node {node_id} is DEAD")
            
            # Send heartbeat to known nodes
            for node_id, status in self.known_nodes.items():
                if status.is_alive:
                    # In real implementation, send heartbeat packet
                    pass
    ...
    def add_known_node(self, node_id: str, ip: str, port: int):
        ...
        self.known_nodes[node_id] = NodeStatus()
